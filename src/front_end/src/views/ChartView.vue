...
import { get_line_data, get_pie_data } from "@/request/api"
import { ILineData, IPieData } from "@/type/charts"

let charts: echarts.ECharts[] = []

const series_list = ref<ILineData[]>()
const xAxis_data = ref<string[]>()


//调用线图接口
get_line_data().then((res: any) => {
    // 初始化
    xAxis_data.value = res.x_axis
    series_list.value = []
    const legend = []
    let info: { [key: string]: ILineData } = {};

    // 判断是否有值
    if (xAxis_data.value?.length === 0) {
        ElMessageBox.alert('获取不到盗版信息', '线图说明', {
    ...
    // 获取值
    for (const val in res.content) {
        const tmp = res.content[val]
        if (tmp.source in info) {
            info[tmp.source].data.push(tmp.count)
        } else {
            info[tmp.source] = {
                name: tmp.source,
                stack: "总数",
    ...
    for (const val in info) {
        legend.push(val)
        series_list.value.push(info[val])
    }

    // 写入图表
    line_chart(xAxis_data.value, legend, series_list.value)
})

// 调用饼图接口
get_pie_data().then((res: any) => {
    let pie_data: IPieData[] = []
    pie_data = res.content
    // 判断是否有值
    if (res.content.length === 0) {
        ElMessageBox.alert('获取不到盗版信息', '饼图说明', {
            confirmButtonText: 'OK'
        })
        return
    }
...
function line_chart(xAxis_data: string[] | undefined, legend_data: string[], series_data: ILineData[]) {
    // 绘制图表左1
    ...
    var option = {
        ...
        xAxis: {
            ...
            type: 'category',
            nameGap: 25,
            data: xAxis_data, //x轴数据
        },
        legend: {
            data: legend_data //线的名字
        },
        yAxis: {
            name: "数量",
            nameLocation: "center",
...
function pie_chart(pie_data: IPieData[]) {
    //图标背景
    const piePatternSrc =
